...
class HybridAccountDashboardUiCases:
    def test_ui_separates_graphics_and_hides_toggle_controls(self):
        ...
        self.assertIn("function queuedLabel", js)
        self.assertIn("if(isRuntimeSuspect(a))return'Checking'", js)
        self.assertIn("if(st==='QUEUED')return'queued';if(luaWaiting(a)&&(a.pid||st==='IN_GAME'||rs==='RUNNING'))return'recovery'", js)
        self.assertIn("if(apiLabel==='Queued'||st==='QUEUED')return queuedLabel(a);if(luaWaiting", js)
        self.assertIn("queue_position", js)
        self.assertIn("ACCOUNT_FILTER==='queued')visible.sort", js)
        self.assertNotIn("Queued #", js)
