...
class HybridAccountLuaHelperCases:
    def test_lua_rejoin_helper_is_served_with_scoped_token_and_local_endpoint(self):
        ...
        self.assertIn("local function warnCronus(message)", script)
        self.assertIn('if source:sub(1, 1) == \\"{\\" then return nil end', script)
        self.assertNotIn('return warnCronus(\\"Launcher not responding\\")', script)
        self.assertNotIn("Cronus teleport reload failed", script)
        ...
        self.assertIn('if eventName ~= "heartbeat" then', script)
        self.assertIn('local quiet = eventName == "heartbeat"', script)
        self.assertIn("if not quiet then", script)
        ...
        self.assertIn('server_type = "PUBLIC"', script)
        ...
        self.assertIn('if source:sub(1, 1) == "{" then', loader)
        self.assertNotIn('return failDownload("Launcher not responding")', loader)
