...
class ArchitectureDisciplineTests(unittest.TestCase):
    ...
    def test_dashboard_stylesheet_stays_split(self):
        ...
        imports = re.findall(r'@import\s+url\("\./styles/([^"?]+)\?v=limiter-single-action"\);', manifest_text)
    ...
    def test_dashboard_uses_native_module_entrypoint(self):
        ...
        dashboard = (ROOT / "ui" / "app" / "dashboard.js").read_text(encoding="utf-8")
        ...
        self.assertIn('<script type="module" src="/ui/app.js?v=developer-copy-no-hang"></script>', index)
        self.assertIn("import './app/dashboard.js?v=developer-copy-no-hang';", app)
        ...
        self.assertIn("from '../panels/settingsPanels.js?v=limiter-single-action'", dashboard)
