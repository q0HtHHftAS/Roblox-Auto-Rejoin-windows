...
_INSTANCE_TOKEN = secrets.token_urlsafe(32)
...
async def api_app_shutdown(request: Request):
    ...
    if not token or not secrets.compare_digest(token, _INSTANCE_TOKEN):
...
def run_with_tray():
    ...
    try:
        ...
        menu = pystray.Menu(
            ...
            Item("â–¶  Start Farm",     lambda: (farm.start() if not farm.running else None)),
            Item("â–   Stop Farm",      lambda: (farm.stop() if farm.running else None)),
            Item("ðŸ” Restart Farm",   lambda: (farm.stop() or time.sleep(0.5) or farm.start())),
